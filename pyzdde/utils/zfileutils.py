...
def readZRD(file_name, file_type):
    """ 
    readZRD(filename, file_type)
    
    Import a zrd file to an array of zemax_ray()
    ...
    Returns
    -------
    zrd : An array of zemax rays. The stored parameters are defined in the zemax_ray class and depend on the file type
...
def writeZRD(rayArray, file_name,file_type):
    """ 
    writeZRD(rayArray, file_name, file_type)
    
    Write an array of zemax_ray() to a zrd file. The uncompressed mode can only be used if all required data is available. Therefore this function can be used to convert an uncompressed zrd file to a compressed file but not vice versa.
    
    Parameters
    ----------
    rayArray : [zemax_ray()] list of zemax_ray elements to be saved
    file_name: [string] name of the zrd file to be imported
    file_type: [string] type of the zrd file ('uncompressed' of 'compressed')
